...
def _detect_predicted_language(token_ids):
    for token_id in token_ids:
        decoded = tokenizer.decode([token_id]).strip()
        if not decoded:
            continue
        if decoded in _SPECIAL_LANG_TOKENS:
            continue
...
def _prepare_start_symbol(lang_code: Optional[str]):
    prefix = "<|startoftranscript|>"
    if lang_code:
        token_ids = tokenizer.encode(f"{prefix}<|{lang_code}|>")
    else:
...
def _decode_prediction(out_tokens):
    if len(out_tokens) <= 3:
        return ""
    return tokenizer.decode(out_tokens[3:-1]).strip().lower()


def _decode_reference(text: str, lang_code: str):
    templ = f"<|startoftranscript|><|{lang_code}|><|transcribe|><|notimestamps|>{text}<|endoftext|>"
    token_ids = tokenizer.encode(templ)
    if len(token_ids) <= 5:
        return text.strip().lower()
    return tokenizer.decode(token_ids[4:-1]).strip().lower()
...
def run_sequence(faces, model, visual_encoder, lang_hint: Optional[str]):
    chunk_frames = args.chunk_size * 25
    preds = []
    detected_lang = None

    start_symbol = _prepare_start_symbol(lang_hint)
    ...
    for i in range(0, faces.size(2), chunk_frames):
        ...
        with torch.no_grad():
            with autocast('cuda', enabled=args.fp16):
                beam_outs, _ = test_scores.forward_pass(
                    model=model,
        ...
        out_tokens = out.cpu().numpy().tolist()
        if detected_lang is None:
            detected_lang = _detect_predicted_language(out_tokens)
        pred_segment = _decode_prediction(out_tokens)
        preds.append(pred_segment)

    full_pred = ' '.join([segment for segment in preds if segment])
    return detected_lang, full_pred
...
if __name__ == '__main__':
    ...
    for index, video_path in progress:
        ...
        lang_hint = ref_lang or args.lang_id
        try:
            pred_lang, pred_text = run_sequence(faces, model, visual_encoder, lang_hint)
        except Exception as exc:
        ...
        if target_lang != "unknown" and predicted_lang == target_lang:
            lang_entry["hits"] += 1

        if ref_text and target_lang != "unknown":
            pred_tokens_words = pred_text.strip().lower().split()
            gt_normalized = _decode_reference(ref_text, target_lang)
            gt_tokens_words = gt_normalized.split()

            video_errors = test_scores.levenshtein(pred_tokens_words, gt_tokens_words)
            video_words = len(gt_tokens_words)
        ...
        metrics_entry = {
            ...
            "pred_lang": predicted_lang,
            "prediction": pred_text,
            "reference": ref_text,
            "has_reference": bool(ref_text) and target_lang != "unknown",
            "errors": video_errors,
            "words": video_words,
            "wer": video_wer,
        }
    ...
    if total_samples:
        ...
        print(f"Language accuracy: {total_hits}/{total_samples} ({total_samples}개 중 {total_hits}개, {accuracy:.2f}%)")
    ...
    if lang_stats:
        ...
        for lang in sorted(lang_stats.keys()):
            ...
            if samples:
                ...
                acc_str = f"accuracy {hits}/{samples} ({samples}/{hits})"
            else:
