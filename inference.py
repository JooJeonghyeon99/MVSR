...
if __name__ == '__main__':
	...
	# Minimal WER print: look for a sibling .txt next to the input video
	base, _ = os.path.splitext(args.fpath)
	ref_txt = base + '.txt'
	ref_text = _parse_reference_text(ref_txt) if os.path.exists(ref_txt) else None

	# Unified output block (no duplicates)
	print("-------------------------------------------------------------------")
	print("-------------------------------------------------------------------")
	...
	if ref_text:
		...
		print(f"WER: {wer*100:.2f}%  (S={S}, D={D}, I={I}, N={N})")
	else:
...
python /mnt/aix7804/multivsr/inference.py \
  ...
  --fpath /mnt/aix7804/multivsr/dataset/data/videos_preprocessed/wWYiw-sEwQc/00006.mp4 \
  --lang_id nl
  """
import numpy as np
import torch, cv2, pickle, sys, os
